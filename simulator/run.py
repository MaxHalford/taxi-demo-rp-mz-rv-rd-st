...
#PG = pw.PostgresqlDatabase('model_store', user='postgres', password='postgres',
#                           host=DOCKER_HOST, port=PG_PORT)

import redis
RD = redis.Redis(host=DOCKER_HOST, port=RD_PORT, db=0)
...
if __name__ == "__main__":
    ...
    # Create topics
    # admin = kafka.admin.KafkaAdminClient(bootstrap_servers=[f"{DOCKER_HOST}:{RP_PORT}"])
